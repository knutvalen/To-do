        ...
        if (intent.action == SaveReminderFragment.ACTION_GEOFENCE_EVENT) {
            ...
            if (geofencingEvent.hasError()) {
                val errorMessage = when (geofencingEvent.errorCode) {
                    GeofenceStatusCodes.GEOFENCE_NOT_AVAILABLE -> this.resources.getString(
                        R.string.geofence_not_available
                    )
                    GeofenceStatusCodes.GEOFENCE_TOO_MANY_GEOFENCES -> this.resources.getString(
                        R.string.geofence_too_many_geofences
                    )
                    GeofenceStatusCodes.GEOFENCE_TOO_MANY_PENDING_INTENTS -> this.resources.getString(
                        R.string.geofence_too_many_pending_intents
                    )
                    else -> this.resources.getString(R.string.geofence_unknown_error)
                }
